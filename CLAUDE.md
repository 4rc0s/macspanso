...
The LSP rides the same path: `buildServer.json` points `xcode-build-server` at `build/DerivedData`, so its diagnostics reflect the **last build's world**, not the current disk. After `xcodegen generate`, build once before trusting diagnostics — a file that has never been built cannot be resolved, and a stale index outlives a branch switch.
...
**Known hole: YAML anchors are not preserved, and cannot be fixed on the current architecture.** espanso documents `anchors:` plus `&name`/`*name` aliases as the compact way to share one script across matches ([docs](https://espanso.org/docs/matches/extensions/#anchors-and-aliases)). Yams resolves aliases while composing the node graph and keeps no anchor name on the resulting value, so any save inlines the shared body at every use site — the `anchors:` block and the body both survive and espanso behaves identically, but the user's DRY structure is flattened. **The mitigation is a warning, not a fix**: `MatchEditorForm` shows an inline notice (`fileUsesYAMLAnchors`, checking both the current file and the picked destination) telling the user what saving will do. The blocked fix is a factual trap worth recording precisely: the earlier theory ("thread anchor identity through the Codable layer") is not executable, because Yams' decode hook exists only for *mappings* — a synthetic `.anchorKeyNode` key that `allKeys` filters out — and there is **no anchor surface at all for scalars or sequences**, which is exactly the canonical espanso shape (`args: [python, -c, *script1]`). The encode hook (`Anchor` encoded under the `anchorKeyNode` string) is useless without a decode half, and aliases are already resolved at compose time, so use sites are indistinguishable from literal copies even at the `Node` level. `Emitter.Options.redundancyAliasingStrategy` is not a shortcut — it invents anchor names and aliases subtrees the user never shared. `RoundTripPreservationTests.testYAMLAliasesAreInlinedOnRoundTrip` pins the current behavior with a strict `XCTExpectFailure`, so it turns red if Yams ever grows the missing surface; delete this paragraph when it does.
...
`.match` (nested-match) vars resolve recursively: `preview(of:in:)` looks the `trigger` param up in the matches passed in (the form passes `store.matchFiles.flatMap(\.matches)` — packages included, like espanso's own resolution — and looks up against *saved* content, not the draft). The documented approximation: with duplicate triggers across files, espanso's winner depends on its config load order, which the preview replaces with "first in store order". The recursion carries a per-branch visited set (espanso's renderer has no cycle guard; the preview degrades to a placeholder instead of hanging the editor). Empty `allMatches` means "no lookup context given" and keeps the plain `[match]` placeholder — distinguish this from a genuinely empty config, which shows `not found`. `date`'s `%P` renders lowercase via a private-use sentinel in the ICU pattern (`lowerDayPeriodSentinel`), replaced with the lowercased day period after formatting — ICU has no case-distinct day-period pair to map it to directly.
...
Every `Process` in the codebase (`EspansoProcessManager.run`, `BackupManager`'s `zip`/`unzip`) follows the same shape, and the deviations have each caused a shipped hang: **drain the pipe to EOF before `waitUntilExit()`**, because output past the ~64 KB pipe buffer blocks the child forever; return early if `proc.run()` throws, since our end still holds the write side and the read would never see EOF; and send unread stdout to `FileHandle.nullDevice` rather than an undrained `Pipe()`. `resolveEspansoPaths` (which parses both the match and runtime directories from one `espanso path` call) additionally races a 3s deadline and kills the child, so a wedged espanso binary can't block launch.

`EspansoProcessManager` parses `espanso status` output as strings, verified against espanso v2.4.1 — it's the fragile seam on espanso upgrades. Pass `espansoPath:` to the initializer in tests; for tests that exercise the paused-state tracker, pass `logURL:` too (a temp file standing in for the daemon log).

**`espanso status` reports daemon liveness only.** It is documented as "Check if the espanso daemon is running or not" and prints `espanso is running` whether or not expansion is enabled; no `espanso cmd` subcommand (`enable`/`disable`/`toggle`/`search`) queries that state, and the IPC protocol (`espanso/src/ipc.rs`) offers only request events. `DaemonState` therefore has no `disabled` case, and nothing may branch on one. Code that did shipped three bugs at once: the menu toggle could only ever disable, an expiring snooze never re-enabled, and the "Espanso Enabled" checkmark always read enabled. Turn expansion on or off with an explicit `setExpansions(enabled:)`; when you genuinely mean "flip it", delegate to espanso's own `cmd toggle` (`toggleExpansions()`) rather than reading a state that isn't there.

**The paused state is nonetheless observable — through the daemon log, and display-only.** The worker logs `toggled enabled state, is_enabled = true|false` on every transition (verified against espanso v2.4.1, `espanso-engine/src/process/middleware/disable.rs`), including no-op requests, `cmd toggle`, and espanso's own keyboard toggle — so it covers toggles made outside this app too. The engine's flag is in-memory and initialized `true` on every worker start; a fresh worker's first log line is always its `reading configs from:` banner, which is how `ExpansionStateParser` distinguishes a real restart (state resets to enabled) from a dead worker's late-flushed output. It deliberately ignores `[service(…)]` lines, which carry the same banner text from every `espanso status` run. `EspansoProcessManager.expansionsPaused` carries the result (`nil` = unknown; reset to `nil` while the daemon is stopped) and **only the menu header may consume it** — commands stay explicit, because the reconstruction depends on log shapes that are no more a contract than the `espanso status` strings are. `ExpansionStateParserTests` pins the shapes; re-verify on espanso upgrades. A supported query has been requested upstream ([espanso#2805](https://github.com/espanso/espanso/issues/2805)); if it ships, the log parsing can be dropped in favor of it. Because every request logs a line, commands **self-verify**: `sendExpansionCommand` arms a window (`commandVerificationWindow`, 12s; tests shrink it), any state-changing line from the parser resolves it, and a lapse degrades `expansionsPaused` to `nil` with a console warning — format drift becomes detected breakage rather than a silently confident header. `ExpansionStateParser.consume` returns whether a line changed state; that return value is the verification signal.
...
The status header is a **disabled** `NSMenuItem` (it is information, not a command), and AppKit dims a disabled item's *plain* title — which made "Espanso running" read as if the status check itself were broken. Attributed titles render verbatim even on disabled items, so the header colors its glyphs through `coloredHeader` (green expanding, red stopped, yellow paused). Converting an attributed header back to a plain `title:` silently greys it.
...
- `VariableHelpContent` (`Support/VariableHelpContent.swift`) is the bundled offline quick reference, reachable from three places: a ? beside the Variables section header (visible with zero variables — help must exist before the first card does), a ? per row of the type-picker sheet (popover; hidden for types with no section), and a ? per var card (`VariableHelpSheet`, the codebase's only WKWebView — the picker reuses the bare `VariableHelpWebView` in a popover). Every `VarType.known` case is documented, `.match` included — nested matches are real (espanso's renderer special-cases them in `espanso-render/src/renderer/mod.rs` and the Nested Matches docs cover them) even though there is no `match.rs` in `espanso-render/src/extension/`; the section cites that source difference explicitly. Adding a `VarType` case fails `VariableHelpContentTests` until a help section is decided, and `.unknown` never gets a button.
