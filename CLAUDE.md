...
Three places must spell that regex the same way, and they are easy to fix one at a time: `MatchExpander.preview` (substitution), `MatchValidator.varReferences` (is this a reference?), and `MatchValidator`'s `invalidVarName` check (is this name referenceable?). The expander was widened to allow `\s*` inside the braces while `varReferences` was left at `\{\{(\w+)\}\}`, so for a while `{{ name }}` was interpolated by espanso, filled in by the preview, and checked by nobody — a typo in that form saved clean. When you touch one, touch all three.
